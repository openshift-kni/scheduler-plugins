...
func (f *fakeFilterCache) GetCachedNUMAPlacementInfo(_ string) *numaplacement.EncodedInfo {
...
func TestFilter_PreemptionFlow(t *testing.T) {
	...
	victim := makeGuaranteedPod("default", "victim", containerName, 4, "1Gi")
	...
	t.Run("without preemption (default mode) pod remains unschedulable", func(t *testing.T) {
		cache := &fakeFilterCache{nrt: nrt, numaPlacement: numaPlacement}
		tm := TopologyMatch{nrtCache: cache, preemptionMode: apiconfig.PreemptionDisabled}
	...
	t.Run("without preemption victims (default mode) pod is unschedulable", func(t *testing.T) {
		cache := &fakeFilterCache{nrt: nrt, numaPlacement: numaPlacement}
	...
	t.Run("victims without NUMA placement info preserve the normal Filter behavior - no preemption effect", func(t *testing.T) {
	...
	t.Run("preemption mode is disabled, preserve the normal Filter behavior even if eviction seems like a solution - no preemption effect", func(t *testing.T) {
		cache := &fakeFilterCache{nrt: nrt}
		tm := TopologyMatch{nrtCache: cache, preemptionMode: apiconfig.PreemptionDisabled}
		cycleState := cycleStateWithVictims(t, victim)
	...
	t.Run("preemption with eviction simulation failure results in unschedulable", func(t *testing.T) {
		cache := &fakeFilterCache{nrt: nrt, numaPlacement: numaPlacement}
		tm := TopologyMatch{nrtCache: cache, preemptionMode: apiconfig.PreemptionEnabled}
		// add a second victim that is not in numa placement info
		victim2 := makeGuaranteedPod("ns-2", "victim2", containerName, 4, "1Gi")
		cycleState := cycleStateWithVictims(t, victim2)
...
func TestGetVictimPods(t *testing.T) {
	...
	got, err := getVictimPods(cycleState, apiconfig.PreemptionEnabled)
	if err != nil {
