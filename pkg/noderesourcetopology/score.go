...
func (tm *TopologyMatch) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	klog.V(6).InfoS("scoring node", "logID", logID, "nodeName", nodeName)
	...
	if !ok {
		klog.V(4).InfoS("noderesourcetopology is not valid for node", "logID", logID, "node", nodeName)
		return 0, nil
	}
	if nodeTopology == nil {
		klog.V(5).InfoS("noderesourcetopology was not found for node", "logID", logID, "node", nodeName)
		return 0, nil
	}

	nrtlog.NodeResourceTopology(logID, "noderesourcetopology found", nodeTopology)
...
func scoreForEachNUMANode(logID string, requested v1.ResourceList, numaList NUMANodeList, score scoreStrategyFn, resourceToWeightMap resourceToWeightMap) int64 {
	...
	for _, numa := range numaList {
		...
		klog.V(6).InfoS("numa score result", "logID", logID, "numaID", numa.NUMAID, "score", numaScore)
	}
	return minScore
}
...
func containerScopeScore(pod *v1.Pod, zones topologyv1alpha2.ZoneList, scorerFn scoreStrategyFn, resourceToWeightMap resourceToWeightMap) (int64, *framework.Status) {
	...
	for i, container := range containers {
		...
		klog.V(6).InfoS("container scope scoring", "logID", logID, "container", container.Name, "score", contScore[i])
	}
	finalScore := int64(stat.Mean(contScore, nil))
	klog.V(5).InfoS("container scope scoring final node score", "logID", logID, "finalScore", finalScore)
	return finalScore, nil
}
